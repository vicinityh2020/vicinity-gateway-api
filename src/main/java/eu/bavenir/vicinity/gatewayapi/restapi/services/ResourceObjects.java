...
public class ResourceObjects extends ServerResource {
	...
	/**
	 * Returns the description of an available IoT object, if OID is specified.
	 * 
	 * OR
	 * 
	 ...
	 * Open Gateway API
	 * 
	 * @param oid Object ID string (from request).
	 * @param type String (from request).
	 * @param limit int (from request).
	 * @param own boolean (from request).
	 * 
	 ...
	 * 
	 * @param oid Object ID string (from the request).
	 */
	@Delete
	public void remove() {
		String attrOid = getAttribute(ATTR_OID);
	...
	private String getObjects(String queryType, String queryLimit, String queryOwn){
		...
		// TODO a real user goes here
		Collection<RosterEntry> rosterObjects = xmppController.getRosterEntriesForUser("user0");
